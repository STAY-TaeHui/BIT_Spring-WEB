...
<div id="header">
	<div class="top-wrapper">
		...
		<%-- 
			...
			>> 활용전 <li><a href="${pageContext.request.contextPath}/joinus/login.htm">로그인</a></li>
...
​			<c:if test="${not empty pageContext.request.userPrincipal}">
			...
			<!-- spring taglib -->
			<se:authorize access="!hasRole('ROLE_USER')"><!-- 현재 로그인되어있지않으면 '로그인'출력 -->
				<li><a href="${pageContext.request.contextPath}/joinus/login.htm">로그인</a></li>
			</se:authorize>
			
			<!-- 위의 userPrincipal로 username을 가져오는 코드와 같은코드 -->
			<se:authentication property="name" var="loginuser"/><!-- 로그인되어있으면 그를 loginuser변수에 저장 -->
			
			<se:authorize access="hasAnyRole('ROLE_ADMIN', 'ROLE_USER')"><!-- admin 또는 user 로 로그인되어있다면 -->
